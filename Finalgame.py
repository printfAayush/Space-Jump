...
pygame.time.set_timer(GRAVITY ,1600)
grav = [1,2]#useless
...
def rotate_roc1(roc):
    new_roc = pygame.transform.rotozoom(roc ,-rocket_movement , 1)#1 is scaling factor
    return new_roc
...
def score_display(game_state):
    ...
    if game_state == "game over":
        ...
        Game_over_surface = game_font.render('PRESS',True,(255,255,0))  
        Gameover_rect = Game_over_surface.get_rect(center = (250,270))
        screen.blit(Game_over_surface,Gameover_rect)
        
        space_surface = game_font.render('||SPACE||',True,(255,255,0))  
        space_rect = space_surface.get_rect(center = (740,270))
        screen.blit(space_surface,space_rect)

        highscore_surface = game_font.render(f'High Score:500 ',True,(255,255,255))  
        highscore_rect = highscore_surface.get_rect(center = (800,560))
...
while True:
    ...
    #rocket
        if grav == 1:
            ...
            rotated_roc = rotate_roc1(rocket_surface)
        ...
        if grav == 2:
            ...
            rocket_movement -= gravity
            rotated_roc = rotate_roc2(rocket_surface)
            rocket_rect.centerx += rocket_movement
            screen.blit(rotated_roc,rocket_rect)
            game_active = check_coll(pipe_list)
